...
int main(int argc, char *argv[]) {
	...
	win = XCreateSimpleWindow(display, RootWindow(display, screen), win_x,
			win_y, width, height, border_width, BlackPixel(display, screen),
	...
	XClearWindow(display, win);
			for (i = 0; i < N; i++) {
				for (j = 0; j < N; j++) {
					if(h[i][j][1] <= 20)
					{
						XSetForeground(display,gc,(long) 0x000000); //black
					}
					else if(h[i][j][1] <= 30)
					{
